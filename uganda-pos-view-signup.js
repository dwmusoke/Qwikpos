...
export async function initSignupScreen() {
  ...
  qsa('[data-plan]', grid).forEach((btn) => btn.addEventListener('click', () => {
...
async function onSubmit(e) {
  ...
  if (!payload.businessName || !payload.fullName || !payload.email || password.length < 6) {
    errEl.textContent = 'Please fill in all fields (password must be at least 6 characters).';
  ...
  const { data, error } = await supabase.auth.signUp({ email: payload.email, password });
  if (error) {
  ...
  if (data.session) {
    ...
    toast('Check your email to confirm your account, then sign in to finish setup.', 'success', 8000);
