...
// =====================================================================
import {
  supabase, STATE, $, qsa, escapeHtml, toast, openModal, closeModal, uid,
  toBase, fromBase, fmtMoneyRaw, currencyMeta, refreshProducts, stockFor,
  queueOfflineSale, buildEfrisPayload, hasFeature,
...
function cartLines() {
  return STATE.cart.map((item) => {
    ...
    const lineGross = Math.round((item.qty * unitPrice + Number.EPSILON) * 100) / 100;
...
function cartTotals() {
  ...
  const lineDetails = lines.map((l) => {
    ...
    const vatAmount = Math.round((netLine * rate / (100 + rate) + Number.EPSILON) * 100) / 100;
    vatTotal += vatAmount;
    return { ...l, netLine, vatAmount, vatRate: rate };
  });
  const grandTotal = Math.round((subtotal - discountTotal + Number.EPSILON) * 100) / 100;
  return { lines: lineDetails, subtotal, discountTotal, vatTotal, grandTotal };
}

export async function renderPOS(root) {
  posSaleCurrency = hasFeature('multi_currency') ? STATE.displayCurrency : STATE.business.base_currency;
  root.innerHTML = `
        ...
        <div style="padding:10px 16px 0;">
          ...
          <div class="currency-select-row">
            <select id="pos-currency-select" ${hasFeature('multi_currency') ? '' : 'disabled title="Upgrade to Growth or Pro for multi-currency sales"'}>
              ${(hasFeature('multi_currency') ? STATE.currencies : STATE.currencies.filter((c) => c.code === STATE.business.base_currency))
                .map((c) => `<option value="${c.code}" ${c.code === posSaleCurrency ? 'selected' : ''}>${c.code} — ${escapeHtml(c.name)}</option>`).join('')}
            </select>
          </div>
        </div>
  ...
  $('pos-search').addEventListener('input', (e) => { posSearchTerm = e.target.value.toLowerCase(); renderProductGrid(); });
  $('pos-scan-btn').addEventListener('click', () => $('pos-search').focus());
  qsa('#pos-categories .chip').forEach((chip) => chip.addEventListener('click', () => {
    posActiveCategory = chip.dataset.cat;
    qsa('#pos-categories .chip').forEach((c) => c.classList.remove('active'));
    chip.classList.add('active');
    renderProductGrid();
  }));
  $('pos-clear-cart').addEventListener('click', () => { STATE.cart = []; renderCart(); });
  $('pos-currency-select').addEventListener('change', (e) => { posSaleCurrency = e.target.value; renderCart(); });
  $('pos-customer-select').addEventListener('change', (e) => { STATE.cartCustomerId = e.target.value || null; });
  qsa('#pos-mode-toggle .chip').forEach((chip) => chip.addEventListener('click', () => {
...
function renderProductGrid() {
  ...
  if (posActiveCategory !== 'all') list = list.filter((p) => p.category_id === posActiveCategory);
  if (posSearchTerm) {
    list = list.filter((p) =>
  ...
  qsa('.product-card', grid).forEach((card) => card.addEventListener('click', () => addToCart(card.dataset.id)));
}
...
function addToCart(productId) {
  ...
  if (existing) {
    ...
    STATE.cart.push({
      productId, name: product.name, qty: 1,
      unitPriceBase: Number(product.selling_price), taxCode: product.tax_category_code || 'STD',
      discount: 0,
    });
  }
  renderCart();
...
function renderCart() {
  ...
  discountInputEl.addEventListener('input', (e) => { posDiscountInput = parseFloat(e.target.value) || 0; renderCart(); });
  // preserve focus/cursor after re-render
  if (document.activeElement !== discountInputEl) { /* no-op */ }

  const checkoutBtn = $('pos-checkout-btn');
  if (checkoutBtn) checkoutBtn.addEventListener('click', () => { isQuote ? saveQuotation() : openCheckoutModal(); });
  const holdBtn = $('pos-hold-btn');
  if (holdBtn) holdBtn.addEventListener('click', () => { toast('Sale held. (Kept in this browser tab only.)', 'default'); });
}
  ...
  `, {
    onMount: (modalRoot) => {
      const updateBalance = () => {
        ...
        $('pay-balance-row').innerHTML = `<span>${balance > 0 ? 'Balance Due' : 'Change'}</span><span style="color:${balance > 0 ? 'var(--danger)' : 'var(--brand)'}">${fmtMoneyRaw(Math.abs(balance), posSaleCurrency)}</span>`;
      };
      updateBalance();
...
async function finalizeSale(payments) {
  ...
  const items = lines.map((l) => ({
    ...
    discount: Math.round((l.lineGross - l.netLine + Number.EPSILON) * 100) / 100,
  ...
  const payload = {
    ...
    subtotal, discount_total: discountTotal, vat_total: vatTotal,
    grand_total: grandTotal, grand_total_base: grandTotalBase,
    customer_id: STATE.cartCustomerId || null,
    payment_status: paymentStatus,
    items,
    payments: payments.map((p) => ({
      method: p.method, currency_code: posSaleCurrency,
      amount: p.amount, amount_base: toBase(p.amount, posSaleCurrency),
    })),
  };

  if (!navigator.onLine) {
    queueOfflineSale(payload);
    toast('You are offline — sale saved and will sync automatically.', 'default', 5000);
...
export async function submitSaleToSupabase(payload) {
  ...
  const { error: itemsErr } = await supabase.from('sale_items').insert(
    payload.items.map((it) => ({ ...it, sale_id: sale.id }))
  );
  if (itemsErr) throw itemsErr;

  const { error: paymentsErr } = await supabase.from('payments').insert(
    payload.payments.map((p) => ({ ...p, sale_id: sale.id, received_by: STATE.appUser.id }))
  );
  ...
  try {
    const { data: fiscalNoData } = await supabase.rpc('next_fiscal_invoice_number');
    ...
    const efrisPayload = buildEfrisPayload({
      sale: { ...sale, ...payload }, items: payload.items, business: STATE.business, customer,
      payments: payload.payments, operator: STATE.appUser?.full_name,
    });

    const { data: efrisInvoice } = await supabase.from('efris_invoices').insert({
      business_id: STATE.business.id,
      sale_id: sale.id,
    ...
    if (efrisInvoice) {
      await supabase.from('efris_queue').insert({ efris_invoice_id: efrisInvoice.id, status: 'pending' });
    }
  } catch (e) {
...
async function saveQuotation() {
  ...
  try {
    ...
    toast(`Quotation ${sale.sale_number} saved — find it in the Quotations tab`, 'success', 5000);
    printableModal({ ...sale, items: payload.items }, { docLabel: 'QUOTATION', footNote: expiry ? `Valid until ${expiry}` : '' });
  } catch (err) {
    console.error(err);
    toast('Could not save quotation: ' + err.message, 'error', 5000);
    if (btn) { btn.disabled = false; btn.textContent = `Save Quotation — ${fmtMoneyRaw(grandTotal, posSaleCurrency)}`; }
  }
}
...
function showReceipt(sale) {
  printableModal(sale, { docLabel: 'TAX INVOICE', footNote: 'EFRIS fiscal invoice staged — see EFRIS tab' });
}
...
export function printableModal(sale, opts = {}) {
  ...
  `, {
    onMount: () => {
      $('print-receipt-btn').addEventListener('click', () => printHtml($('receipt-print-area').outerHTML, docLabel === 'QUOTATION' ? 'Quotation' : 'Receipt'));
    },
  });
}
...
export function printHtml(innerHtml, title = 'Document') {
  ...
  w.document.write(`<html><head><title>${title}</title></head><body>${innerHtml}</body></html>`);
  w.document.close();
  w.focus();
  w.print();
}
...
function cartTotalsSnapshot(sale) {
  ...
  const lines = items.map((it) => ({ name: it.product_name, qty: it.quantity, unitPrice: it.unit_price, lineGross: it.line_total }));
  return { lines, subtotal: sale.subtotal, discountTotal: sale.discount_total, vatTotal: sale.vat_total, grandTotal: sale.grand_total };
}
