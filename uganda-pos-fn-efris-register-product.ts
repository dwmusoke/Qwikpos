...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json(result, result.success ? 200 : 400, corsHeaders);
  } catch (err) {
    console.error(err);
    return json(
      { success: false, error: "Internal error during product registration" },
      500,
      corsHeaders,
    );
  }
...
export async function registerProduct(
  ...
  if (!business.efris_live_enabled)
    return {
      success: false,
      error: "Live EFRIS is not enabled for this business yet.",
    };
  ...
  if (!product.efris_commodity_category_id) {
    return {
      success: false,
      error: `"${product.name}" is missing an EFRIS Commodity Category ID — set one in Inventory before registering.`,
    };
  }
  ...
  if (data?.response !== "OK") {
    return {
      success: false,
      error: data?.message || "EFRIS rejected the goods registration",
      raw: data,
    };
  }
