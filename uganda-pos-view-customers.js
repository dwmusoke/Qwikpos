...
import {
  ...
  fmtMoney, refreshCustomers, fmtDate,
...
function renderTable() {
  ...
  qsa('[data-edit]', tbody).forEach((b) => b.addEventListener('click', () => openCustomerModal(b.dataset.edit)));
  qsa('[data-statement]', tbody).forEach((b) => b.addEventListener('click', () => openStatementModal(b.dataset.statement)));
}
...
async function openStatementModal(customerId) {
  ...
  openModal(`<div class="modal-title-row"><h3>Statement — ${escapeHtml(c.name)}</h3></div><div class="empty-state">Loading…</div>`, { large: true });

  const { data: sales } = await supabase.from('sales').select('*').eq('customer_id', customerId).order('created_at', { ascending: false }).limit(50);
    ...
    <div class="table-wrap" style="margin-top:12px; max-height:340px; overflow-y:auto;">
      <table>
        ...
        <tbody>
          ...
          `).join('') : '<tr><td colspan="4"><div class="empty-state">No purchases yet.</div></td></tr>'}
        </tbody>
      </table>
    </div>
