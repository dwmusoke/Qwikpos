...
// =====================================================================
import { supabase, STATE, fmtMoney, fmtDate, lowStockProducts, escapeHtml } from './uganda-pos-core.js';
...
export async function renderDashboard(root) {
  ...
  const [{ data: sales }, { data: efrisRows }, { data: customers }] = await Promise.all([
  ...
  // Quotations aren't real sales yet (unpaid, no EFRIS) — exclude them from every dashboard figure.
  ...
  const sum = (arr, field) => arr.reduce((a, s) => a + Number(s[field] || 0), 0);
  // vat_total is stored per-sale in that sale's own currency — convert to base via its exchange_rate before summing.
  const sumConverted = (arr, field) => arr.reduce((a, s) => a + Number(s[field] || 0) * Number(s.exchange_rate || 1), 0);
  ...
  const inventoryValue = STATE.products.reduce((a, p) => a + Number(p.cost_price || 0) * (STATE.stockByProduct[p.id] || 0), 0);
  const outstandingBalance = (customers || []).reduce((a, c) => a + Number(c.balance || 0), 0);

  // top products (by qty) from last 90 days
  const productTally = {};
  allSales.forEach((s) => (s.sale_items || []).forEach((it) => {
    productTally[it.product_name] = (productTally[it.product_name] || 0) + Number(it.quantity);
  }));
  ...
  root.innerHTML = `
    ...
    <div class="grid-2">
      <div class="card">
        ...
        </table></div>` : `<div class="empty-state">No sales yet — head to <b>Sell (POS)</b> to record your first one.</div>`}
      </div>
