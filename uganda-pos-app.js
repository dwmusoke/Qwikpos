...
// =====================================================================
import {
  supabase, STATE, $, qsa, toast, loadBootstrapData, hasRole, hasFeature, isSubscriptionActive,
  lowStockProducts, offlineQueueCount, flushOfflineQueue,
...
import { initSignupScreen, finishPendingSignupIfAny } from './uganda-pos-view-signup.js';
...
const ROUTES = {
  ...
  reports: { title: 'Reports', render: renderReports, feature: 'reports_export' },
  accounting: { title: 'Accounting', render: renderAccounting, feature: 'accounting' },
  settings: { title: 'Settings', render: renderSettings },
  billing: { title: 'Billing', render: (root) => renderBilling(root, { paywall: false }) },
  admin: { title: 'Platform Admin', render: renderAdmin, superadminOnly: true },
};

async function navigateTo(route) {
  if (!ROUTES[route]) route = STATE.isSuperadmin && !STATE.business ? 'admin' : 'dashboard';

  // Superadmins with no vendor business of their own can only see the admin console.
  if (STATE.isSuperadmin && !STATE.business && route !== 'admin') route = 'admin';
  ...
  qsa('.nav-link').forEach((l) => l.classList.toggle('active', l.dataset.route === route));
...
function wireShell() {
  qsa('.nav-link').forEach((link) => {
    ...
    if (isSuperadminLink && !STATE.isSuperadmin) { link.style.display = 'none'; return; }
    if (roles && !hasRole(...roles.split(','))) { link.style.display = 'none'; return; }

    // A superadmin with no vendor business of their own only needs the admin console.
    if (STATE.isSuperadmin && !STATE.business && route !== 'admin') { link.style.display = 'none'; return; }
  ...
  const currencyOptions = hasFeature('multi_currency') ? STATE.currencies : STATE.currencies.filter((c) => c.code === STATE.business.base_currency);
  picker.classList.remove('hidden');
  picker.innerHTML = currencyOptions.map((c) => `<option value="${c.code}" ${c.code === STATE.displayCurrency ? 'selected' : ''}>${c.code}</option>`).join('');
  picker.disabled = !hasFeature('multi_currency');
  picker.title = hasFeature('multi_currency') ? '' : 'Upgrade to Growth or Pro for multi-currency';
...
function populateUserChip() {
  const initials = (STATE.appUser.full_name || 'U').split(' ').map((s) => s[0]).slice(0, 2).join('').toUpperCase();
...
function updateBadges() {
  ...
  supabase.from('efris_invoices').select('id', { count: 'exact', head: true })
    .eq('business_id', STATE.business.id).in('status', ['pending', 'queued'])
    .then(({ count }) => {
...
function wireConnectivity() {
  ...
  const update = () => {
    ...
    if (online && offlineQueueCount() > 0) {
      flushOfflineQueue(submitSaleToSupabase).then(() => { pill.classList.toggle('show', offlineQueueCount() > 0); });
    }
  };
