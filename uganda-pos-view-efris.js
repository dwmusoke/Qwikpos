...
//     connect a provider.
// =====================================================================
import { supabase, STATE, $, qsa, escapeHtml, toast, openModal, closeModal, fmtMoneyRaw, fmtDate } from './uganda-pos-core.js';
...
export async function renderEfris(root) {
  ...
  root.innerHTML = `
    ...
    <div class="category-chips" style="margin-bottom:14px;">
      ${['all', 'pending', 'queued', 'accepted', 'rejected', 'failed'].map((s) => `
  ...
  const renderRows = () => {
    const list = efrisFilter === 'all' ? invoices : invoices.filter((i) => i.status === efrisFilter);
    const tbody = $('efris-table-body');
    if (!list.length) { tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state">No EFRIS invoices in this filter.</div></td></tr>`; return; }
    ...
    qsa('[data-view]', tbody).forEach((b) => b.addEventListener('click', () => viewPayload(invoices.find((i) => i.id === b.dataset.view))));
    qsa('[data-submit]', tbody).forEach((b) => b.addEventListener('click', () => submitInvoice(b.dataset.submit)));
  };
...
function statusBadge(status) {
  const map = { pending: 'badge-gray', queued: 'badge-blue', accepted: 'badge-green', rejected: 'badge-red', failed: 'badge-red' };
...
async function submitInvoiceLive(invoiceId) {
  ...
  const { data, error } = await supabase.functions.invoke('efris-submit-invoice', { body: { efrisInvoiceId: invoiceId } });

  if (error || !data?.success) {
    toast('EFRIS submission failed: ' + (data?.error || error?.message || 'unknown error'), 'error', 8000);
  } else {
    toast(`EFRIS invoice accepted ✅ FDN: ${data.invoiceNo || ''}`, 'success', 6000);
  }
...
async function submitInvoiceSimulated(invoiceId) {
  ...
  await supabase.from('efris_invoices').update({ status: 'queued' }).eq('id', invoiceId);
  ...
  setTimeout(async () => {
    ...
    const update = success
      ? {
        status: 'accepted',
        antifake_code: 'AF' + Math.random().toString(36).slice(2, 10).toUpperCase(),
        qr_code: 'EFRIS-QR-' + Math.random().toString(36).slice(2, 14).toUpperCase(),
        submitted_at: new Date().toISOString(),
        error_message: null,
      }
      : { status: 'rejected', error_message: 'Simulated: verify buyer TIN and retry.', submitted_at: new Date().toISOString() };

    await supabase.from('efris_invoices').update(update).eq('id', invoiceId);
    await supabase.from('efris_queue').update({ status: success ? 'done' : 'failed', last_error: update.error_message }).eq('efris_invoice_id', invoiceId);

    toast(success ? 'EFRIS invoice accepted ✅ (simulated)' : 'EFRIS invoice rejected — see details', success ? 'success' : 'error');
    document.querySelector('[data-route="efris"]')?.click();
  }, 1200);
}

function exportCsv(invoices) {
  const header = ['Fiscal Invoice No', 'Sale No', 'Customer', 'TIN', 'Currency', 'Gross Amount', 'VAT Amount', 'Status', 'Date'];
  const rows = invoices.map((i) => [
    i.fiscal_invoice_number, i.sales?.sale_number || '', i.customer_name || '', i.customer_tin || '',
    i.currency_code, i.gross_amount, i.vat_amount, i.status, i.created_at,
  ]);
  const csv = [header, ...rows].map((r) => r.map((v) => `"${String(v ?? '').replace(/"/g, '""')}"`).join(',')).join('\n');
  const blob = new Blob([csv], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `efris-invoices-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
