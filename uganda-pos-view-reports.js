...
// =====================================================================
import { supabase, STATE, $, escapeHtml, fmtMoney, fmtDate } from './uganda-pos-core.js';
...
export async function renderReports(root) {
  ...
  async function runReport() {
    ...
    const { data: sales } = await supabase.from('sales').select('*, sale_items(*), payments(*), cashier:app_users!cashier_id(full_name)')
      .eq('business_id', STATE.business.id)
      .gte('created_at', `${from}T00:00:00`).lte('created_at', `${to}T23:59:59`)
      .order('created_at', { ascending: false });

    // Quotations aren't real sales (unpaid, no stock/EFRIS impact) — keep them out of every report figure.
    lastSales = (sales || []).filter((s) => s.status !== 'voided' && s.sale_type !== 'quotation');
    ...
    const totalSales = lastSales.reduce((a, s) => a + Number(s.grand_total_base), 0);
    const totalVat = lastSales.reduce((a, s) => a + Number(s.vat_total || 0) * Number(s.exchange_rate || 1), 0);
    const totalDiscount = lastSales.reduce((a, s) => a + Number(s.discount_total || 0) * Number(s.exchange_rate || 1), 0);

    const paymentTotals = {};
    lastSales.forEach((s) => (s.payments || []).forEach((p) => { paymentTotals[p.method] = (paymentTotals[p.method] || 0) + Number(p.amount_base || 0); }));

    const productTally = {};
    lastSales.forEach((s) => (s.sale_items || []).forEach((it) => {
      if (!productTally[it.product_name]) productTally[it.product_name] = { qty: 0, revenue: 0 };
      productTally[it.product_name].qty += Number(it.quantity);
      productTally[it.product_name].revenue += Number(it.line_total) * Number(s.exchange_rate || 1);
    }));
    const topProducts = Object.entries(productTally).sort((a, b) => b[1].revenue - a[1].revenue).slice(0, 8);

    const cashierTally = {};
    lastSales.forEach((s) => {
      const key = s.cashier_id || 'unknown';
      if (!cashierTally[key]) cashierTally[key] = { name: s.cashier?.full_name || 'Unknown', txns: 0, revenue: 0, items: 0 };
      cashierTally[key].txns += 1;
      cashierTally[key].revenue += Number(s.grand_total_base || 0);
      cashierTally[key].items += (s.sale_items || []).reduce((a, it) => a + Number(it.quantity || 0), 0);
    });
    const cashierRanking = Object.values(cashierTally).sort((a, b) => b.revenue - a.revenue);
    const medal = ['🥇', '🥈', '🥉'];
    ...
    output.innerHTML = `
      ...
      <div class="card">
        ...
        </div>` : '<div class="empty-state">No sales in this range.</div>'}
      </div>
